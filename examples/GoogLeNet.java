...
@SuppressWarnings("UnnecessaryLocalVariable")
	public static Model create() {
		Model model = new Model.Builder().add(
			...
			new Inception.Builder().filterAmount(64, 96, 16, 128, 32, 32).initializer(new Constant(0)).build()
		).add(
			new Inception.Builder().filterAmount(128, 128, 32, 192, 96, 64).initializer(new Constant(0)).build()
		).add(
			new Pooling.Builder().downsampleSize(3).downsampleStride(2).mode(Pooling.Mode.MAX).build()
		).add(
			new Inception.Builder().filterAmount(192, 96, 16, 208, 48, 64).initializer(new Constant(0)).build()
		).add(
			new Inception.Builder().filterAmount(160, 112, 24, 224, 64, 64).initializer(new Constant(0)).build()
		).add(
			new Inception.Builder().filterAmount(128, 128, 24, 256, 64, 64).initializer(new Constant(0)).build()
		).add(
			new Inception.Builder().filterAmount(112, 144, 32, 288, 64, 64).initializer(new Constant(0)).build()
		).add(
			new Inception.Builder().filterAmount(256, 160, 32, 320, 128, 128).initializer(new Constant(0)).build()
		).add(
			new Pooling.Builder().downsampleSize(3).downsampleStride(2).mode(Pooling.Mode.MAX).build()
		).add(
			new Inception.Builder().filterAmount(256, 160, 32, 320, 128, 128).initializer(new Constant(0)).build()
		).add(
			new Inception.Builder().filterAmount(384, 192, 48, 384, 128, 128).initializer(new Constant(0)).build()
		).add(
			...
			new Dense.Builder().outputSize(1000).activation(OutputActivationType.SOFTMAX).initializer(new Constant(0))
				.temperature(1).build()
		).cost(CostType.SPARSE_CROSS_ENTROPY).updaterType(UpdaterType.AMSGRAD).inputDimensions(224, 224, 3).build();
